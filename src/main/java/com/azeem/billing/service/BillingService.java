...
public class BillingService {
    ...
    public Page<String> getAvailableBillingPeriods(int page, int size) {
        Pageable periodsRequest = PageRequest.of(
                page,
                size,
                Sort.by("billingPeriod").descending()
        );
    ...
    public Page<BillingRecord> getRecordsByPeriod(String billingPeriod, int page, int size) {
        Pageable periodRequest = PageRequest.of(page,
                size,
                Sort.by("billingPeriod").descending()
        );
        return repository.findByBillingPeriod(billingPeriod, periodRequest)
    ...
    public Page<BillingRecord> getAllRecords(int page, int size) {
        ...
        Page<BillingRecord> records = repository.findAll(pageable)
                .map(mapper::mapToDomain);
    ...
    public Page<BillingRecord> getRecordsByDepartment(@NotBlank String department,
                                                      int page,
                                                      int size) {
        Pageable recordsByDepartmentRequest = PageRequest.of(page,
                size,
                Sort.by("department").descending()
        );
    ...
    public Page<BillingRecord> getTopNRecords(@Min(1) int n) {
        ...
        Page<BillingRecord> records = repository.findAll(topNRequest)
                .map(mapper::mapToDomain);
    ...
    public BillingSummary generateSummaryForPeriod(@NotBlank String billingPeriod) {
        List<BillingRecord> records = repository.findByBillingPeriod(billingPeriod)
                .stream()
                .map(mapper::mapToDomain)
                .toList();
